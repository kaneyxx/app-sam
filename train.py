...
def eval_psnr(loader, model, eval_type=None):
    ...
    if eval_type == 'f1':
        ...
        metric1, metric2, metric3, metric4 = 'f1', 'none', 'none', 'none'
    ...
    #     batch_pred = [torch.zeros_like(pred).cpu().numpy() for _ in range(dist.get_world_size())]
    #     batch_gt = [torch.zeros_like(batch['gt']).cpu().numpy() for _ in range(dist.get_world_size())]

    #     dist.all_gather(batch_pred, pred.detach().cpu().numpy())
    #     pred_list.extend(batch_pred)
    #     dist.all_gather(batch_gt, batch['gt'].detach().cpu().numpy())
